...
func (c Client) RetrievePredictionsForStops(stopIDs []string) (*PredictionList, error) {
	fmt.Println("Retrieving Predictions for StopIDs", stopIDs)
	...
	q.Add("key", c.APIKey)
	q.Add("format", "json")
	q.Add("stpid", strings.Join(stopIDs, ","))
	q.Add("top", "100")
	req.URL.RawQuery = q.Encode()
